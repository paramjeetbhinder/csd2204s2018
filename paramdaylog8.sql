...
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near '> from products
    -> left join manu
    -> on products.manufacturer = manu.cod' at line 2
MariaDB [csd2204s18]> select products.code,products.name,manu.name
    -> from products
    -> left join manu
    -> on products.manufacturer = manu.code;
...
MariaDB [csd2204s18]> select products.code,products.name,manu.name
    ...
    -> from products
    -> right join manu
    -> on prodeucts.code = manu.code;
ERROR 1054 (42S22): Unknown column 'prodeucts.code' in 'on clause'
MariaDB [csd2204s18]> select products.code,products.name,manu.name
    -> from products
    -> left join manu
    -> on products.code = manu.code
...
MariaDB [csd2204s18]> select count(*),manufacturer
    -> from products as p
    -> inner join manu m
    -> in m.code = p.manufacturer
    -> group by manufacturer;
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'in m.code = p.manufacturer
group by manufacturer' at line 4
MariaDB [csd2204s18]> select count(*),manufacturer
    -> from products as p
    -> inner join manu m
...
MariaDB [csd2204s18]> select count(*),p.manufacturer,m.name
    -> from products as p
    -> inner join manu m
    -> on m.code = p.manufacturer
    -> group by manufacturer;
...
MariaDB [csd2204s18]> select count(*),p.manufacturer,m.name
    ...
    -> inner join manu m
    -> on m.code = p.manufacturer
    -> group by manufacturer having manufacturer<4;
+----------+--------------+-----------------+
| count(*) | manufacturer | name            |
+----------+--------------+-----------------+
|        1 |            1 | Sony            |
...
MariaDB [csd2204s18]> select count(*),p.manufacturer,m.name
    ...
    -> inner join manu m
    -> on m.code = p.manufacturer
    -> group by manufacturer having manufacturer in (2,3);
+----------+--------------+-----------------+
| count(*) | manufacturer | name            |
+----------+--------------+-----------------+
|        3 |            2 | Creative Labs   |
